...
function onMouseUp() {
    ...
    TextRank(text,{extractAmount: 1});


    a.textContent = summarizedArticle;
    a.style = "color:black; background-color: rgb(37, 150, 190); border-radius: 10px; padding: 5px"
    a.id = "summary";
    console.log(summarizedArticle);

    node = document.getSelection().anchorNode.parentElement;

    console.log(node.querySelectorAll("#summary"));
    node.querySelectorAll("#summary").forEach( (element) => {
      console.log("what the heck man");
      element.remove();
    })
    if(a.textContent != "" && a.textContent.length > 5 && node.id != "summary"){
      node.appendChild(a);
    }
...
//api key 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

async function GPT3TEST(){
  console.log("SENT REQUEST TO OPEN AI!!!!!!!!!!!!!!")
  const response = await fetch("https://api.openai.com/v1/completions", {
  method: 'POST',
  headers: {
    'Content-Type' : 'application/json',
    'Authorization': 'Bearer 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'
  },
  body: `{
      "model": "text-davinci-003",
      "prompt": "Say this is a test",
